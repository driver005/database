...
func (m Migrator) CreateTable(values ...interface{}) (err error) {
	...
	for _, value := range m.ReorderModels(values, false) {
		if err = m.RunWithValue(value, func(stmt *database.Statement) error {
...
func (m Migrator) GetRows(currentSchema interface{}, table interface{}) (*sql.Rows, error) {
	...
	return m.DB.Session(&database.Session{}).Table(name).Limit(1).Scopes(func(d *database.DB) *database.DB {
...
func (m Migrator) getColumnSequenceName(tx *database.DB, stmt *database.Statement, field *schema.Field) (
...
func (m Migrator) resetPreparedStmts() {
	if m.DB.PrepareStmt {
		if pdb, ok := m.DB.ConnPool.(*database.PreparedStmtDB); ok {
			pdb.Reset()
		}
	}
}
