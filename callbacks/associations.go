...
func SaveBeforeAssociations(create bool) func(db *database.DB) {
...
func SaveAfterAssociations(create bool) func(db *database.DB) {
	return func(db *database.DB) {
		if db.Error == nil && db.Statement.Schema != nil {
			selectColumns, restricted := db.Statement.SelectAndOmitColumns(create, !create)
...
func saveAssociations(db *database.DB, rel *schema.Relationship, rValues reflect.Value, selectColumns map[string]bool, restricted bool, defaultUpdatingColumns []string) error {
	...
	if tx.Statement.FullSaveAssociations {
		tx = tx.Set("database:update_track_time", true)
	}
...
var visitMapStoreKey = "database:saved_association_map"

func checkAssociationsSaved(db *database.DB, values reflect.Value) bool {
