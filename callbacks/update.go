...
func SetupUpdateReflectValue(db *database.DB) {
...
func BeforeUpdate(db *database.DB) {
...
// Update update hook
func Update(config *Config) func(db *database.DB) {
...
func AfterUpdate(db *database.DB) {
...
func ConvertToAssignments(stmt *database.Statement) (set types.Set) {
	...
	default:
		...
		if !updatingValue.CanAddr() || stmt.Dest != stmt.Model {
			// different schema
			updatingStmt := &database.Statement{DB: stmt.DB}
