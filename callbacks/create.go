...
func BeforeCreate(db *database.DB) {
	if db.Error == nil && db.Statement.Schema != nil && !db.Statement.SkipHooks && (db.Statement.Schema.BeforeSave || db.Statement.Schema.BeforeCreate) {
		callMethod(db, func(value interface{}, tx *database.DB) (called bool) {
...
// Create create hook
func Create(config *Config) func(db *database.DB) {
...
func AfterCreate(db *database.DB) {
...
func ConvertToCreateValues(stmt *database.Statement) (values types.Values) {
	...
	default:
		var (
			...
			_, updateTrackTime        = stmt.Get("database:update_track_time")
			isZero                    bool
		)
		...
		case reflect.Slice, reflect.Array:
			...
			for i := 0; i < rValLen; i++ {
				...
				if !rv.IsValid() {
					stmt.AddError(fmt.Errorf("slice data #%v is invalid: %w", i, database.ErrInvalidData))
					return
				}
		...
		default:
			stmt.AddError(database.ErrInvalidData)
		}
	}
