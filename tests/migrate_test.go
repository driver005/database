...
func TestSmartMigrateColumn(t *testing.T) {
	...
	columnclause, err := DB.Table("user_migrate_columns").Migrator().Columnclause(&UserMigrateColumn{})
	if err != nil {
		t.Fatalf("failed to get column clause, got error: %v", err)
	}
...
func TestMigrateColumns(t *testing.T) {
	...
	if columnclause, err := DB.Migrator().Columnclause(&ColumnStruct{}); err != nil {
		t.Fatalf("no error should returns for Columnclause")
	} else {
...
func TestMigrateColumnOrder(t *testing.T) {
	...
	columnclause, err := DB.Table("user_migrate_columns").Migrator().Columnclause(&UserMigrateColumn2{})
	if err != nil {
		t.Fatalf("failed to get column clause, got error: %v", err)
	}
	typ := reflect.Indirect(reflect.ValueOf(&UserMigrateColumn2{})).Type()
	numField := typ.NumField()
	if numField != len(columnclause) {
		t.Fatalf("column's number not match struct and ddl, %d != %d", numField, len(columnclause))
	}
	namer := schema.NamingStrategy{}
	for i := 0; i < numField; i++ {
		expectName := namer.ColumnName("", typ.Field(i).Name)
		if columnclause[i].Name() != expectName {
			t.Fatalf("column order not match struct and ddl, idx %d: %s != %s",
				i, columnclause[i].Name(), expectName)
		}
	}
}
...
func findColumnType(dest interface{}, columnName string) (
	...
	columnclause, err := DB.Migrator().Columnclause(dest)
	if err != nil {
		err = fmt.Errorf("Columnclause err:%v", err)
		return
	}
...
func TestDifferentTypeWithoutDeclaredLength(t *testing.T) {
	...
	err = DB.Table("diff_clause").AutoMigrate(&DiffType1{})
	if err != nil {
		t.Errorf("AutoMigrate err:%v", err)
	}
